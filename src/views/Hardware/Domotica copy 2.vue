				...
				<n-gi v-for="device in devices" :key="device.deviceId">
					<n-card :title="device.nome || device.deviceId" :bordered="false">
						...
						<n-space vertical>
							...
							>
							<n-space
								v-if="device.coverState"
								justify="space-between"
								class="cover-state-row"
								><span class="cover-state">{{
									stateLabel(device.coverState)
								}}</span></n-space
							>
							<n-progress
								type="line"
...
export default {
	...
	methods: {
		...
		async refreshCoverStatuses() {
			...
			try {
				...
				this.devices.forEach((device) => {
					const key = `${device.deviceId}:${device.coverId ?? 0}`;
					const status = statusByKey.get(key);
					if (!status) return; // nessun dato ancora ricevuto via MQTT per questo device

					if (status.currentPos >= 0) device.position = status.currentPos;
					device.coverState = status.state;
				});
			} catch (err) {
				// Un fallimento occasionale non deve bloccare la UI.
		...
		isMoving(device) {
			return device.coverState === "opening" || device.coverState === "closing";
		},
		canCommand(device) {
		...
		async setPosition(device, position) {
			...
			try {
				// ⚠️ Il backend vuole il numero direttamente come "command" (es. "75"),
				// non la parola "position" con un campo separato.
				const response = await sendCoverCommand(device, String(position));
				if (!response?.success)
